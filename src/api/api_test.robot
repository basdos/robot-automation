...
Given user hit api request login
    ...
    ${json_body}=    Set Variable    {"username": "emilys","password": "emilyspass"}
    ${requestBody}=    Evaluate    json.loads("""${json_body}""")    json
    Log To Console    ${requestBody}
    ${response}=    Post Request    sessionLogin    /auth/login    json=${requestBody}
    # ${response}=    POST On Session    sessionLogin    /auth/login    json=${requestBody}
